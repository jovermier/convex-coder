...
function isBackendRunning() {
  try {
    ...
    const containers = JSON.parse(`[${psResult.trim().split("\n").join(",")}]`);
    const backend = containers.find((c) => c.Service === "backend");
    return (
      backend && (backend.State === "running" || backend.Health === "healthy")
    );
  } catch (error) {
    return false;
  }
...
function getAdminKeyFromBackend() {
  ...
  console.log(
    "⚠️  Could not get admin key using generate_admin_key.sh, trying direct approach..."
  );
  ...
  if (fs.existsSync(ENV_DOCKER_FILE)) {
    try {
      ...
      if (dockerParsed.variables.NEXT_PUBLIC_ADMIN_KEY !== adminKey) {
        ...
        writeEnvFileWithFormatting(
          ENV_DOCKER_FILE,
          dockerParsed.lines,
          dockerParsed.variables
        );
  ...
  try {
    ...
    if (updatedFiles > 0) {
      ...
      if (fs.existsSync(ENV_DOCKER_FILE)) {
        console.log("");
        console.log(
          "💡 If the dashboard is already running, restart it to pick up the new key:"
        );
...
function showStatus() {
  ...
  if (fs.existsSync(ENV_DOCKER_FILE)) {
    try {
      ...
      const dockerStatus =
        dockerKey === "placeholder" || dockerKey === "app|placeholder"
          ? "❌ Placeholder"
          : "✅ Set";
  ...
  if (fs.existsSync(ENV_LOCAL_FILE)) {
    try {
      ...
      const localStatus =
        localKey === "placeholder" ? "❌ Placeholder" : "✅ Set";
