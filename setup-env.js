...
function setupDockerEnv() {
  ...
  if (process.env.PGURI) {
    // Remove database name from URL as Convex backend adds it internally
    const pgUrl = new URL(process.env.PGURI);
    // Remove database name from pathname - Convex handles this internally
    pgUrl.pathname = "";

    const postgresUrlWithoutDb = pgUrl.toString();

    updates.POSTGRES_URL = postgresUrlWithoutDb;

    // Check if PostgreSQL SSL certificate exists (should be generated by generate-postgres-cert.sh)
    const fs = require("fs");
    const certPath = "/tmp/postgres_ca_chain.pem";

    if (fs.existsSync(certPath) && fs.statSync(certPath).size > 0) {
      console.log("🔐 Using existing PostgreSQL SSL certificate...");
    } else {
      console.log("⚠️  PostgreSQL SSL certificate not found!");
      console.log(
        "💡 Run 'pnpm generate-postgres-cert' first to generate the certificate"
      );
      console.log("🔧 Falling back to SSL disabled mode...");

      // Fall back to SSL disabled mode
      updates.DO_NOT_REQUIRE_SSL = "1";
      updates.PGSSLMODE = "disable";
      console.log("📊 Database: SSL disabled - fallback mode");
      return;
    }
